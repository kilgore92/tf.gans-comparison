...
def read_parse_preproc(filename_queue,image_size,dataset=''):
...
def get_batch_join(tfrecords_list, batch_size, shuffle=False, num_threads=1, min_after_dequeue=None, num_epochs=None,image_size=64,dataset=''):
    name = "batch_join" if not shuffle else "shuffle_batch_join"
    with tf.variable_scope(name):
        filename_queue = tf.train.string_input_producer(tfrecords_list, shuffle=shuffle, num_epochs=num_epochs)
        example_list = [read_parse_preproc(filename_queue,image_size=image_size,dataset=dataset) for _ in range(num_threads)]
...
# interfaces
def shuffle_batch_join(tfrecords_list, batch_size, num_threads, num_epochs, min_after_dequeue=None,image_size = 64,dataset=''):
    return get_batch_join(tfrecords_list, batch_size, shuffle=True, num_threads=num_threads,
        num_epochs=num_epochs, min_after_dequeue=min_after_dequeue,image_size=image_size,dataset=dataset)
